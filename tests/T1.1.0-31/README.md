# T1.1.0-31 aligned top and bottom framing

This test retains T30's three-screen interface and raises the title/version pair by one row. Their leading spaces are removed so the top-left title framing aligns with the bottom-left screen identity. `X ROMS FOUND` retains a one-character content margin like the other loading-status lines.
...
3. Run `ONEROMT11031`, make no selection change, and press Q.
...
4. Confirm the title starts in the top-left cell with no empty row or left margin. Confirm the bottom-left labels change from `IEC SELECT` to `LOAD` and then `ROM SELECT`, while the lower-right credit remains intact.
...
The tokenized physical PRG is 21,606 bytes.
