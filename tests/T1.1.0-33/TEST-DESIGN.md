...
T33 changes the ROM redraw helper from row offset `I+5` to `I+4`. T32 moved the initial menu up one row by preventing the clear-screen print from advancing, so the cursor-positioned incremental redraw must move up by the same amount.
