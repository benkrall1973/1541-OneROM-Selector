# T1.1.0-27 clean Save exit and IEC row redraw

This test retains T26's compact flicker-free ROM menu and makes two behavioral improvements. The VICE S=Save simulation now ends the program at BASIC `READY.` instead of returning to the menu, matching the intended physical-hardware flow. The IEC address selector now repaints only its old and new highlighted address rows after Up/Down.
...
3. Run `ONEROMT11027`, make no selection change, and press Q.
...
3. Move between devices 8 and 9 and confirm only the two address rows change, with no list redraw or flash. Select either address with RETURN.
4. Confirm the screen displays one `SEARCHING...` line followed by `ONEROM FOUND IN LOWER SOCKET`.
5. Exercise ROM-menu movement and confirm the blank line below `ACTIVE:` remains visible.
6. Press S and confirm `SIMULATED SAVE COMPLETE`, `CLEAN EXIT COMPLETE`, and BASIC `READY.` appear without returning to the menu.
7. Run it again without resetting VICE, press Q, confirm the boot-pair checkpoint, and finish at BASIC `READY.`.
...
The tokenized physical PRG is 21,490 bytes.
