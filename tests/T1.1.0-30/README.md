# T1.1.0-30 named screens and compact ROM result

This test retains T29's accurate staged loading display and adds the agreed screen identities: `IEC SELECT`, `LOAD`, and `ROM SELECT`. They are written directly to the lower-left screen-memory cells while `BUILT WITH CHATGPT` remains aligned at lower right.
...
3. Run `ONEROMT11030`, make no selection change, and press Q.
...
4. Confirm the bottom-left labels change from `IEC SELECT` to `LOAD` and then `ROM SELECT`, while the lower-right credit remains intact.
