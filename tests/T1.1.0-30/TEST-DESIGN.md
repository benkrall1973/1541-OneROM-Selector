...
T30 retains T29's operation boundaries. It removes only the redundant startup ROM-name printing; the names are still retrieved into `N$()` and shown on `ROM SELECT`.
