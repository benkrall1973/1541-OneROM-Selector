# T1.1.0-26 corrected menu-row redraw

This test corrects T25's two-row redraw position. T25 targeted each ROM row one screen line too high after navigation, overwriting the intended blank line beneath `ACTIVE:`. T26 targets the actual list rows, preserving the blank line during every selection change.
...
3. Run `ONEROMT11026`, make no selection change, and press Q.
...
4. Exercise menu movement and confirm the blank line below `ACTIVE:` always remains visible and only the old and new selection rows change.
...
The tokenized physical PRG is 21,282 bytes.
