...
IEC Select and Load established their bottom framing before populating their bodies, while ROM Select waited until the body was complete. T35 standardizes that visible construction order without changing the footer implementation or any communication logic.

The countdown always renders two digits (`10` through `01`), so its width remains 19 characters and its centered coordinates do not move while counting. This is a coordinate-only presentation change.

The screenshot showed the initial active row remaining highlighted while a second bar appeared below the selected ROM. This was not intentional active-ROM highlighting. It was an exact one-row addressing mismatch between the full menu and the incremental redraw helper. T33 realigns the helper without changing selection state or the `ACTIVE:` display.

The centered headers use fixed padding because both strings have fixed widths. This adds no cursor-positioning routine and cannot affect the direct-memory footer. Removing punctuation changes strings only; IEC, RBCP, and clean-exit code remain identical to T31.

The earlier blank top row was produced by `PRINT CHR$(147)` completing a print operation and advancing the cursor after clearing the screen. `PRINT CHR$(147);` clears without advancing. This is a presentation-only change and does not touch IEC, RBCP, menu, or clean-exit behavior.

T30 changes presentation only. The removed ROM-name output loop did not perform discovery or retrieval; it merely printed values already stored in `N$()`. The lower-left screen identities use the same direct screen-memory principle as the proven footer, avoiding the cursor/scrolling failure mode encountered by earlier footer experiments.
