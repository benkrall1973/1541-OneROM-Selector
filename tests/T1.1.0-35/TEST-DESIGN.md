...
T35 moves the existing `GOSUB 7000` footer call from after the completed ROM menu to immediately after the title/version. Because the routine writes screen memory directly, moving the call does not change the BASIC cursor position or menu-row coordinates.
