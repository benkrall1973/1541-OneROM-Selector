# T1.1.0-35 consistent screen construction order

This test retains T34's interface and draws the `ROM SELECT` bottom bar immediately after its title/version, before populating the menu body. All three primary screens now establish their top and bottom framing before drawing changing content.
...
3. Run `ONEROMT11035`, make no selection change, and press Q.
...
3. Confirm `AUTO SELECT IN [10]` is centered and updates in place. Confirm the IEC instruction line reads `UP/DOWN=SELECT   RETURN=CONTINUE`. Move between devices 8 and 9 and confirm only the two address rows change, with no list redraw or flash. Select either address with RETURN.
...
6. Confirm `ROM SELECT` and `BUILT WITH CHATGPT` appear before `ACTIVE`, the ROM rows, and controls populate. Exercise movement and confirm exactly one candidate row is highlighted, the old row is restored, and the blank line below `ACTIVE:` remains visible.
...
The tokenized physical PRG is 21,672 bytes.
