...
T29 instruments startup presentation at the actual BASIC control-flow boundaries. The first large upload is identified before its `M-W` sequence. The upper probe is identified before execution; an upper failure reports the mapping-specific rewrite before the lower probe. The service configuration, start, response retrieval, ROM list, and menu transition are then reported in order.
