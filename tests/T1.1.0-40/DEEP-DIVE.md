...
T21's apparent second-run RAM value 255 had a separate parsing cause: valid slot zero is a NUL byte, which BASIC `GET#` returns as an empty string. Initializing `RA` to 255 made zero look missing. T40 initializes it to zero and still rejects an explicit byte 255.
