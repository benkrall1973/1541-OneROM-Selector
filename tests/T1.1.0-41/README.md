# T1.1.0-41 load-screen spacing polish

T41 preserves T40's hardware-proven repeated clean-exit behavior and makes
three presentation-only adjustments:

- adds a one-character left margin before every bottom-left screen name;
- removes the blank line after `ONEROM FOUND IN LOWER SOCKET`;
- adds a blank line between `X ROMS FOUND` and `BOOTING TO MENU`.
...
2. Run `ONEROMT11041` and verify the three spacing changes on IEC Select, Load,
   and ROM Select.
...
T35 remains the VICE interface-simulation build. T41 is intended for physical-drive testing.
